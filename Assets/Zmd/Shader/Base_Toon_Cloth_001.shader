Shader "Zmd/Base_Toon_Cloth_001"
{
    Properties
    {
        [Header(Main)]
        ...
        _AnisoMaskTex("Aniso Mask Tex", 2D) = "white" {}
        _Tex2("Tex2", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // Cloth: Tex2 darkening
                float2 tex2UV = float2(i.uv.x * 32.1, i.uv.y);
                float4 tex2Color = tex2D(_Tex2, tex2UV);
                float3 mainTex_Dark = mainTex.rgb * float3(0.115, 0.115, 0.115);
                float3 fac = saturate(tex2Color.rgb);
                mainTex.rgb = lerp(mainTex.rgb, mainTex_Dark, fac);
                ...
                // Aniso Mask: UV → Mapping(Scale:56.1,1,1) → R channel
                float2 anisoMaskUV = float2(i.uv.x * 56.1, i.uv.y);
                float4 anisoMaskTex = tex2D(_AnisoMaskTex, anisoMaskUV);
                float anisoMaskVal = anisoMaskTex.r;
                ...
                spe = lerp(spe, float3(anisoMaskVal, anisoMaskVal, anisoMaskVal), 1);
                ...
                float Vertical_Atten = i.worldNormal.y * 0.5 + 0.5; // Blender Z(up) → Unity Y(up)
                ...
                return float4(finalcol, mainTex.a);
            }
            ENDHLSL
        }
