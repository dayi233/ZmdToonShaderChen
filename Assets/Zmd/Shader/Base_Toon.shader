    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                //Rim                
                float3 RimColor = _RimColor.rgb * _RimColorStrength;
                ...
                //Clip
                float mask = _IsSkin > 0.5;
                mask = lerp(mainTex.a, 1, saturate(mask));
                //output
                finalcol = finalcol * 1.1;
                // Mix Shader: A=Transparent, B=finalcol, Fac=mask → premultiplied alpha
                clip(mask - 0.5);
                return float4(finalcol * mask, mask);
