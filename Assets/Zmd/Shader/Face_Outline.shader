    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                // B: Mix(MULTIPLY) — lerp(col, col*0.017, 0.85)
                col = lerp(col, col * float4(0.017, 0.017, 0.017, 1), 0.85);
        ...
        Pass
        {
            ...
            half4 frag_outline(v2f i, bool isFrontFace : SV_IsFrontFace) : SV_Target
            {
                ...
                // === B: 沿用本 shader 的暗色 lerp 逻辑 ===
                col = lerp(col, col * float4(0.017, 0.017, 0.017, 1), 0.85);
                col.rgb *= (0.2 + light.shadowAttenuation * 0.8);
                col *= 1.2;

                float3 color = diffuse + specular + col.rgb;
